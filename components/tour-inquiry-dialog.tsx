...
━━━━━━━━━━━━━━━━━━━━
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // 1. Procesamos las actividades para que no salgan solo IDs
    const actividadesTexto = formData.activities
      .map(id => {
        if (id === 'otro') return formData.activitiesOther;
        const opt = activitiesOptions.find(o => o.id === id);
        return opt ? opt.label : id;
      })
      .filter(Boolean)
      .join(", ");

    // 2. Creamos el mensaje estructurado con TUS datos
    const message = `*NUEVA SOLICITUD DE COTIZACIÓN - PARADISAEA* ✈️
-----------------------------------
👤 *CLIENTE:* ${formData.firstName} ${formData.lastName}
📧 *EMAIL:* ${formData.email}
📞 *TEL:* ${formData.phone}

📍 *DESTINO:* ${formData.destination || tour?.name || "Interés general"}
👥 *PASAJEROS:* ${formData.travelers}
📅 *SALIDA:* ${formData.departureDay}/${formData.departureMonth}/${formData.departureYear}
📅 *REGRESO:* ${formData.returnDay}/${formData.returnMonth}/${formData.returnYear}

🎒 *TIPO:* ${formData.tripType === 'otro' ? formData.tripTypeOther : formData.tripType}
🗺️ *ACTIVIDADES:* ${actividadesTexto || "Por definir"}
💰 *PRESUPUESTO:* ${formData.budget}
📱 *CONTACTO:* ${formData.contactPreference.toUpperCase()}

_Enviado desde el portal web de Paradisaea_`

    // 3. Envío directo a tu WhatsApp
    const whatsappUrl = `https://api.whatsapp.com/send?phone=525535013294&text=${encodeURIComponent(message)}`
    window.open(whatsappUrl, "_blank")

    // 4. Envío de Email (Mantenemos la lógica para que v0 la use después)
    try {
      await sendEmail({
        to: "ventas.paradisaea@gmail.com",
        subject: `Nueva Solicitud: ${formData.firstName} - ${tour?.name || formData.destination}`,
        body: message,
      })
    } catch (error) {
      console.log("Error enviando copia al email:", error)
    }

    // 5. Limpieza y cierre automático
    setIsSubmitting(false)
    setFormData({ ...emptyFormData })
    onOpenChange(false) 
    alert("¡Listo! Tu información ha sido enviada. Se abrirá WhatsApp para que nos contactes.")
  }
